...
#define FATAL(...) \
    do { \
        fprintf(stderr, "SysCall Detective: " __VA_ARGS__); \
        fputc('\n', stderr); \
        exit(EXIT_FAILURE); \
    } while (0)
...
printf("%s\n","The SysCall Detective program has the command format SysCall Detective [-h] [-f file] command [args].  OPTIONAL: \nThe [-h] is an help summary to user, if an user add invalid argument the help summary will print. \nThe [-f file] it’s an representation of file or folder location in computer system.\nThe [args] command line arguments, arguments do modification for commands.\nMANDATORY Command – is the tracee: For example, this can be a predefined command in linux or other executable programs.");
return 1;    
